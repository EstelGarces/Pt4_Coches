...
public class PrometeoCarController : MonoBehaviour
{
    ...
    //float lateralGrip = 1f;             // Agarre lateral
    //float rollingResistance = 0.01f;    // Resistencia al rodamiento
    ...
    */
    ...
    void Update()
    {
        ...
        {
            ActivateCamera(0); // Activa la primera cámara
        }
        else if (Input.GetKeyDown(KeyCode.Alpha2))
        {
            ActivateCamera(1); // Activa la segunda cámara
        }


        CheckWheelSurface(frontLeftCollider);
        CheckWheelSurface(frontRightCollider);
        CheckWheelSurface(rearLeftCollider);
        CheckWheelSurface(rearRightCollider);
    ...
    void CheckWheelSurface(WheelCollider wheelCollider)
    {
        RaycastHit hit;

        // Dispara un Raycast hacia abajo desde el WheelCollider para detectar el suelo.
        if (Physics.Raycast(wheelCollider.transform.position, Vector3.down, out hit, 1f))
        {
            // Llama a la función para ajustar el agarre dependiendo de la capa detectada.
            GridAjust(hit.collider);
        }
    }


    void GridAjust(Collider collider)
    {
        if (collider.gameObject.layer == LayerMask.NameToLayer("Road"))
        {
            accelerationMultiplier = 4;  // Velocidad normal en asfalto
        }
        else if (collider.gameObject.layer == LayerMask.NameToLayer("Terrain"))
        {
            accelerationMultiplier = 1;  // Velocidad reducida en tierra
        }
    }
    // This method converts the car speed data from float to string, and then set the text of the UI carSpeedText with this value.
    public string CarSpeedUI()
    {

        //if(useUI){
        //try{
        float absoluteCarSpeed = Mathf.Abs(carSpeed);
        string carSpeedText = Mathf.RoundToInt(absoluteCarSpeed).ToString();
